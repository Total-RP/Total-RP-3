...
local function refreshTooltip(Frame)
	if Frame.titleText and Frame.GenFrame and Frame.GenFrameX and Frame.GenFrameY and Frame.GenFrameAnch then
		...
		GameTooltip:SetOwner(Frame.GenFrame, Frame.GenFrameAnch,Frame.GenFrameX,Frame.GenFrameY);
		GameTooltip:AddLine(Frame.titleText, 1, 1, 1, true);
		GameTooltip:AddLine(Frame.bodyText, 1, 0.6666, 0, true);
		GameTooltip:Show();
	end
end
	...
	local listCallback = function(value)
		for index, tab in pairs(values) do
			...
			if val == value then
				...
				-- If this is a font dropdown, stylize each item with the font it describes
...
function Storyline_API.lib.getDefaultFont()
	local defaultFont = LSM3:GetDefault(LSM3.MediaType.FONT);
	local locale = Storyline_Data.config.locale;
	local error; -- Not used for now. Stores the errors while fetching the default font

	if locale == "koKR" then
		-- If the chosen locale is Korean, first check that the font is actually present
		if LSM3:IsValid(LSM3.MediaType.FONT, "기본 글꼴") then
			defaultFont = LSM3:Fetch(LSM3.MediaType.FONT, "기본 글꼴")
		elseif LSM3:IsValid(LSM3.MediaType.FONT, "2002") then -- Western and russian clients have the Korean font as 2002
			defaultFont = LSM3:Fetch(LSM3.MediaType.FONT, "2002")
		else
			error = [[Current language set to Korean, but no default font supporting Korean characters is available. Check the text options to define a valid font manually.]]
		end
	elseif locale == "zhCN" then
		-- If the chosen locale is Simplified Chinese, first check that the font is actually present
		if LSM3:IsValid(LSM3.MediaType.FONT, "默认") then
			defaultFont = LSM3:Fetch(LSM3.MediaType.FONT, "默认")
		elseif LSM3:IsValid(LSM3.MediaType.FONT, "AR ZhongkaiGBK Medium") then -- Western and russian clients have the Simplified Chinese font as AR ZhongkaiGBK Medium
			defaultFont = LSM3:Fetch(LSM3.MediaType.FONT, "AR ZhongkaiGBK Medium")
		else
			error = [[Current language set to Simpilified Chinese, but no default font supporting Simpilified Chinese characters is available. Check the text options to define a valid font manually.]]
		end
	elseif locale == "zhTW" then
		-- If the chosen locale is Traditionnal Chinese, first check that the font is actually present
		if LSM3:IsValid(LSM3.MediaType.FONT, "預設") then
			defaultFont = LSM3:Fetch(LSM3.MediaType.FONT, "預設")
			-- No other client have the Traditionnal Chinese font :(
		else
			error = [[Current language set to Traditionnal Chinese, but no default font supporting Traditionnal Chinese characters is available. Check the text options to define a valid font manually.]]
		end
	end
