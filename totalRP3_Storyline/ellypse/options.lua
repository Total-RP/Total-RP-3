...
local function setOptionColor(affectedText, optionKey, r, g, b)
	Storyline_Data.config[optionKey].Color = {
		r = r,
		g = g,
		b = b
	}
	affectedText:SetTextColor(r, g, b, 1);
	StorylineTextOptionsPanel[optionKey].TextSample:SetTextColor(r, g, b, 1);
	StorylineTextOptionsPanel[optionKey].ColorSwatch.Background:SetVertexColor(ColorPickerFrame:GetColorRGB());
end

local function ShowColorPicker(optionKey, affectedText)
	local function callback(r, g, b)
		setOptionColor(affectedText, optionKey, r, g, b)
	end
	ColorPickerFrame.func, ColorPickerFrame.opacityFunc, ColorPickerFrame.cancelFunc = callback, callback, callback;
	local r, g, b = Storyline_Data.config[optionKey].Color.r, Storyline_Data.config[optionKey].Color.g, Storyline_Data.config[optionKey].Color.b;
...
local function decorateTextOptions(title, optionKey, affectedText)
	...
	StorylineTextOptionsPanel[optionKey].SizeSlider:SetScript("OnValueChanged", function(_, scale)
	...
	if not Storyline_Data.config[optionKey].FontFlag then
		Storyline_Data.config[optionKey].FontFlag = '';
	end

	setupListBox(StorylineTextOptionsPanel[optionKey].FontFlagsDropDown, fontFlags, function(fontFlag)
		local font, scale, _ = affectedText:GetFont();
		affectedText:SetFont(font, scale, fontFlag);
		StorylineTextOptionsPanel[optionKey].TextSample:SetFont(font, scale, fontFlag);
		Storyline_Data.config[optionKey].FontFlag = fontFlag;
	end, nil, 100, true);
	StorylineTextOptionsPanel[optionKey].FontFlagsDropDown:SetSelectedValue(Storyline_Data.config[optionKey].FontFlag);

	if not Storyline_Data.config[optionKey].Color then
		Storyline_Data.config[optionKey].Color = {
			r = 1,
			g = 1,
			b = 1
	...
	StorylineTextOptionsPanel[optionKey].ColorSwatch:SetScript('OnClick', function(self)
		ShowColorPicker(optionKey, affectedText);
	end);
	setOptionColor(affectedText, optionKey, Storyline_Data.config[optionKey].Color.r, Storyline_Data.config[optionKey].Color.g, Storyline_Data.config[optionKey].Color.b);
end
...
Storyline_API.options.init = function()
	...
	local localeTab = {};
	for localeIndex, localeInfo in pairs(Storyline_API.locale.getLocales()) do
		tinsert(localeTab, {localeInfo.localeText, localeIndex});
	end
	...
	StorylineOptionsPanel.TextSpeed.Slider:SetScript("OnValueChanged", function(_, speed)
	...
	local function onUpdateTextSpeedSample(_, elapsed)
