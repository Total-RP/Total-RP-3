...
local loc = TRP3.loc;
local EMPTY = TRP3.globals.empty;
...
local DEFAULT_RELATIONS = {
	...
	UNFRIENDLY = { id = "UNFRIENDLY", order = 1, texture = TRP3_InterfaceIcons.RelationUnfriendly, color = TRP3.RelationColors.Unfriendly:GenerateHexColorOpaque() },
	NEUTRAL = { id = "NEUTRAL", order = 2, texture = TRP3_InterfaceIcons.RelationNeutral, color = TRP3.RelationColors.Neutral:GenerateHexColorOpaque() },
	BUSINESS = { id = "BUSINESS", order = 3, texture = TRP3_InterfaceIcons.RelationBusiness, color = TRP3.RelationColors.Business:GenerateHexColorOpaque() },
	FRIEND = { id = "FRIEND", order = 4, texture = TRP3_InterfaceIcons.RelationFriend, color = TRP3.RelationColors.Friend:GenerateHexColorOpaque() },
	LOVE = { id = "LOVE", order = 5, texture = TRP3_InterfaceIcons.RelationLove, color = TRP3.RelationColors.Love:GenerateHexColorOpaque() },
	FAMILY = { id = "FAMILY", order = 6, texture = TRP3_InterfaceIcons.RelationFamily, color = TRP3.RelationColors.Family:GenerateHexColorOpaque() },
};
...
local function getRelationList(sorted)
	local relationList = TRP3.configuration.getValue("register_relation_list");
	if not relationList then
		relationList = CopyTable(DEFAULT_RELATIONS);
		TRP3.configuration.setValue("register_relation_list", relationList);
	end
...
TRP3.register.relation.getRelationList = getRelationList;
...
TRP3.register.relation.getRelationInfo = getRelationInfo;
...
TRP3.register.relation.setRelation = setRelation;

local function getRelation(profileID)
	local relationTab = TRP3.profile.getData("relation") or EMPTY;
	return getRelationInfo(relationTab[profileID]);
end
TRP3.register.relation.getRelation = getRelation;
...
TRP3.register.relation.getRelationText = getRelationText;
...
local function getRelationTooltipText(profileID, profile)
	...
	local player = TRP3.register.getPlayerCompleteName(true);
	local target = TRP3.register.getCompleteName(profile.characteristics or EMPTY, UNKNOWN, true);
	return GenerateFormattedDescription(description, player, target);
end
TRP3.register.relation.getRelationTooltipText = getRelationTooltipText;

local function getRelationTexture(profileID)
	return getRelation(profileID).texture;
end
TRP3.register.relation.getRelationTexture = getRelationTexture;

local function getRelationColor(profileID)
	local relation = getRelation(profileID);
	if relation.color then
		return TRP3.CreateColorFromHexString(relation.color);
	end
end
TRP3.register.relation.getRelationColor = getRelationColor;

local function getColor(relation)
	local relationColor = getRelationInfo(relation).color;
	if relationColor then
		return TRP3.CreateColorFromHexString(relationColor);
	end
end
TRP3.register.relation.getColor = getColor;
...
local function pasteCopiedIcon(frame)
	local icon = TRP3.GetLastCopiedIcon() or TRP3_InterfaceIcons.ProfileDefault;
...
local function checkRelationUse()
	...
	local profiles = TRP3.profile.getProfiles();
	for _, profile in pairs(profiles) do
		local relations = TRP3.profile.getData("relation", profile);
...
local function initRelationEditor(relationID)
	...
	setTooltipAll(TRP3_RelationsList.Editor.Content.Icon, "RIGHT", 0, 5, loc.UI_ICON_SELECT, TRP3.FormatShortcutWithInstruction("LCLICK", loc.UI_ICON_OPENBROWSER) .. "|n" .. TRP3.FormatShortcutWithInstruction("RCLICK", loc.UI_ICON_OPTIONS));
	TRP3_RelationsList.Editor.Content.Icon:SetScript("OnClick", function(self, button)
		if button == "LeftButton" then
			TRP3.popup.showPopup(TRP3.popup.ICONS, nil, {function(icon)
				draftRelationTexture = icon;
		...
		elseif button == "RightButton" then
			...
			TRP3_MenuUtil.CreateContextMenu(self, function(_, description)
				description:CreateButton(loc.UI_ICON_COPY, TRP3.SetLastCopiedIcon, draftRelationTexture);
				description:CreateButton(loc.UI_ICON_COPYNAME, function() TRP3.popup.showCopyDropdownPopup({draftRelationTexture}); end);
	...
	if relation.color then
		TRP3_RelationsList.Editor.Content.Color.setColor(TRP3.CreateColorFromHexString(relation.color):GetRGBAsBytes());
	else
		TRP3_RelationsList.Editor.Content.Color.setColor(nil);
	end
end

function TRP3.register.relation.showEditor(relationID)
...
local function onActionSelected(selectedAction)
	...
	local originalRelation = (getColor(relation) or TRP3.Colors.White)(relation.name or loc:GetText("REG_RELATION_" .. relation.id));
	if action == ACTIONS.EDIT then
		TRP3.register.relation.showEditor(relation.id);
	elseif not relation.inUse and action == ACTIONS.DELETE then
		TRP3.popup.showConfirmPopup(loc.CO_RELATIONS_DELETE_WARNING:format(originalRelation), function()
			local relationList = getRelationList();
...
function updateRelationsList()
	...
	for _, relation in ipairs(relations) do
		...
		widget.Title:SetText((getColor(relation) or TRP3.Colors.White)(relation.name or loc:GetText("REG_RELATION_"..relation.id)));
		...
		TRP3.ui.tooltip.setTooltipForSameFrame(widget.Actions, "RIGHT", 0, 5, loc.CM_OPTIONS, TRP3.FormatShortcutWithInstruction("CLICK", loc.CM_OPTIONS_ADDITIONAL));
...
local function saveCurrentRelation()
	if not TRP3.utils.str.emptyToNil(TRP3_RelationsList.Editor.Content.Name:GetText()) then
		TRP3.utils.message.displayMessage(loc.CO_RELATIONS_NEW_ERROR, TRP3.utils.message.type.ALERT_MESSAGE);
		return
	end
	...
	if TRP3_RelationsList.Editor.Content.Color.red and TRP3_RelationsList.Editor.Content.Color.green and TRP3_RelationsList.Editor.Content.Color.blue then
		relationToUpdate.color = TRP3.CreateColorFromBytes(TRP3_RelationsList.Editor.Content.Color.red, TRP3_RelationsList.Editor.Content.Color.green, TRP3_RelationsList.Editor.Content.Color.blue):GenerateHexColorOpaque();
	else
...
local function onRelationSelected(value)
	...
	if hasProfile(unitID) then
		TRP3.register.relation.setRelation(hasProfile(unitID), value);
...
local function onTargetButtonClicked(_, _, _, button)
	TRP3_MenuUtil.CreateContextMenu(button, function(_, description)
		local relations = TRP3.register.relation.getRelationList(true);
...
TRP3.register.inits.relationsInit = function()
	local configDefault = CopyTable(DEFAULT_RELATIONS);
	TRP3.configuration.registerConfigKey("register_relation_list", configDefault);
	...
	TRP3.RegisterCallback(TRP3_Addon, TRP3_Addon.Events.WORKFLOW_ON_LOADED, function()
		...
		TRP3.target.registerButton({
			...
			adapter = function(buttonStructure, unitID)
				...
				buttonStructure.tooltipSub = TRP3.register.relation.getRelationTooltipText(profileID, getProfile(profileID)) .. "\n\n" .. TRP3.FormatShortcutWithInstruction("CLICK", loc.REG_RELATION_TARGET);
				buttonStructure.icon = TRP3.register.relation.getRelationTexture(profileID);
			end,
		});
	end);
	...
	TRP3.RegisterCallback(TRP3_Addon, TRP3_Addon.Events.WORKFLOW_ON_FINISH, function()
		...
		TRP3.ui.tooltip.setTooltipForSameFrame(TRP3_RelationsList.Editor.Content.Description.help, "RIGHT", 0, 5, loc.CO_RELATIONS_DESCRIPTION, loc.CO_RELATIONS_DESCRIPTION_TT);
		...
		TRP3.navigation.menu.registerMenu({
			...
			onSelected = function()
				TRP3.navigation.menu.selectMenu(RELATIONS_MENU_ID);
			end,
		});
		TRP3.navigation.menu.registerMenu({
			...
			onSelected = function()
				TRP3.navigation.page.setPage(RELATIONS_PAGE_ID);
				TRP3_RelationsList.ScrollFrame.Content:SetWidth(TRP3_RelationsList.ScrollFrame:GetWidth());
			end,
		});
	end)

	TRP3.RegisterCallback(TRP3_Addon, TRP3_Addon.Events.NAVIGATION_RESIZED, function(_)
		TRP3_RelationsList.ScrollFrame.Content:SetWidth(TRP3_RelationsList.ScrollFrame:GetWidth());
	end);
end
