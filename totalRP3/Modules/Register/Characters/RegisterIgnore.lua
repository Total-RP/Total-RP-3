...
TRP3.register.isIDIgnored = isIDIgnored;
...
TRP3.register.ignoreID = ignoreID;
...
TRP3.register.ignoreIDConfirm = ignoreIDConfirm;

local function getIgnoreReason(unitID)
	return blockList[unitID];
end
TRP3.register.getIgnoreReason = getIgnoreReason;

function TRP3.register.getIDsToPurge()
...
function TRP3.register.unignoreID(unitID)
	blockList[unitID] = nil;
	TRP3_Addon:TriggerEvent(Events.REGISTER_DATA_UPDATED, unitID, TRP3.register.isUnitIDKnown(unitID) and hasProfile(unitID) or nil, nil);
end

function TRP3.register.getIgnoredList()
	return blockList;
end
...
TRP3.RegisterCallback(TRP3_Addon, Events.WORKFLOW_ON_LOAD, function()
	...
	if TRP3.target then
		...
		TRP3.target.registerButton({
			...
			condition = function(_, unitID)
				return UnitIsPlayer("target") and unitID ~= TRP3.globals.player_id and not isIDIgnored(unitID);
			end,
			onClick = function(unitID)
				ignoreIDConfirm(unitID);
			end,
			tooltipSub = loc.TF_IGNORE_TT .. "\n\n" .. TRP3.FormatShortcutWithInstruction("CLICK", loc.TF_IGNORE),
