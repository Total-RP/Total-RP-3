...
|cff00ff00Essas informações serão exibidas na tela de informações do seu personagem.]=],
		...
		GEN_NEW_VERSION_AVAILABLE = [=[Uma nova versão do Total RP 3 está disponível.
|cffff0000Sua versão: %s
|c0000ff00Nova versão: %s|r

|cffff9900Nós fortemente o encorajamos a ficar atualizado.|r

Essa mensagem só irá aparecer uma vez por sessão e pode ser desativada nas opções.]=],
...
Se não quiser indicar a personalidade do seu personagem desta forma, mantenha esta caixa desmarcada e a janela de personalidade ficará completamente escondida.]=],
		...
		REG_PLAYER_STYLE_BATTLE_2 = "Batalha de dados do TRP (Não disponível ainda)",
...
O Total RP não é responsável por links que levem a conteúdo nocivo.
		...
		WHATS_NEW = [=[{h3:c}Novidades na versão {col:6eff51}1.0.1{/col}{/h3}
{h3}1. Coordenadas de residência{/h3}
Você pode agora colocar a residência do seu personagem no mapa usando o botão "determinar posição" ao editar características.
Jogadores agora podem ver essa marca no mapa ao ler seu perfil.

{h3}2. Expurgo automático de diretório{/h3}
WoW não é livre de bugs, e um bem irritante é que quando os dados salvos por um addon atingem um certo nível, é tudo apagado.
Para reduzir as chances de atingir esse nível, implementamos um expurgo automático de diretório que remove perfis de personagens que você nunca encontrou ou encontrou há mais de 5 dias e não atribuiu nenhuma relação.
Você pode mudar este parâmetro {link*scandisable*aqui}.

{h3:c}Novo na versão {col:6eff51}1.0{/col}{/h3}
{h3}1. Sistema de localização de personagens{/h3}
Você agora pode escanear por localizações de personagens no seu mapa. {link*map*Abra seu mapa} e use o botão no canto inferior esquerdo para escanear!
|cffff9900Não quer estar visível? {link*scandisable*Você pode desativar o escaneio}.

{h3}2. Nova UI para edição "À primeira vista"{/h3}
Nós melhoramos a maneira como você pode editar a informação dos seus slots "À primeira vista". {link*glance*Dê uma olhada!}

{h3}3. Novo botão de mudança de linguagem{/h3}
Um novo botão foi adicionado à barra de ferramentas para rapidamente mudar entre as linguagens conhecidas por seu personagem. {link*language*Dê uma olhada!}]=],
	}
