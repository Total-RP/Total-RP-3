...
{h3}TipTac module{/h3}
Le |cff9999ffmodule TipTac|r ajouter la compatibilité avec l'add-on TipTac pour modifier l'apparence et la position des infobulles de Total RP 3.
|cff9999ffDisponible sur Curse: {link*storyline*http://mods.curse.com/addons/wow/total-rp-3-tiptac-module}]=],
...
|cffffff00Glisser-déposer:|r Réorganiser les emplacements]=], -- Needs review
...
Total RP n'est pas responsable du contenu des liens partagés.]=],
        ...
        WHATS_NEW = [=[{h3:c}Nouveautés dans la version {col:6eff51}1.0.4{/col}{/h3}
Cette version contient principalement des corrections de bogues et des optimisations. Nous travaillons d'arrache-pied sur {link*TRP3E*Total RP 3: Extended} pour vous apporter les meilleur outils de création pour World of Warcraft!

{h3}Insérer le nom roleplay complet dans la fenêtre de discussion{/h3}
Vous pouvez maintenant MAJ-Cliquer sur le nom d'un personnage dans la fenêtre de discussion pour insérer son nom roleplay complet. Vous pouvez insérer le nom normal en ALT-MAJ-Cliquant sur le nom. Vous pouvez désactiver cette option dans les {link*chat_settings*paramètres de discussion}.

Nous avons également ajouter une nouvelle {link*chat_settings*méthode d'affichage des noms} dans la fenêtre de discussion pour afficher le titre court dans le nom du joueur.]=],
    }
};
